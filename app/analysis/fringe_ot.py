...
# 「算得准」的条纹数门槛。规范 §3 的 DEFAULTS 里是 3.0，这里按平台的实际
# 样品下调到 2.0，作为**显式 override** 传进去 —— DEFAULTS 一个字不改，
# 块 A 回显的是本次实际用的值（规范本来就这么要求）。
#
# 为什么是 2.0：775–1120 nm 这个窗口下，一个频率 bin 是 2516 nm，
# 3 条纹对应 OT ≈ 3774 nm。而真实的钙钛矿湿膜从 7500 nm 干燥到 2000 nm 左右，
# 大半段时间根本到不了 3 条纹 —— 拿 3.0 卡，整条曲线会几乎全被判成
# 「不准」，那个标注就失去了区分度，等于没标。
#
# 2.0 条纹对应 OT ≈ 2516 nm，落在干燥后期的实际厚度上，能把
# 「测得准的那一段」和「已经薄到测不准的那一段」真正分开。
# 代价说清楚：2–3 条纹之间的精度确实不如 ≥3，规范 §4.1 给的实测对照是
# cycles 2.37 → 偏差 -4.12%。所以这一档在界面上写的是「可用」，不是「精确」。
PLATFORM_ACCURATE_CYCLES = 2.0

# 三档判级在界面上怎么说人话。key 是 status_of() 的返回值。
STATUS_TEXT = {
    "OK": {
        "label": "可信",
        "short": "窗内条纹数够，FFT 锁在真峰上",
        "detail": (
            f"窗口内至少有 {PLATFORM_ACCURATE_CYCLES:g} 个完整的干涉条纹，"
            "而且峰值信噪比达标。这一档的光学厚度可以直接用。\n\n"
            "「条纹数」= 窗口的频率跨度 ÷ 一个频率 bin，"
            "物理上就是这段波长范围里数得出几个明暗周期。数得越多，"
            "FFT 定峰越准。"),
    },
    "DEGRADED": {
        "label": "可用但精度下降",
        "short": "条纹数在 1.5–2 之间，能定峰但误差变大",
        "detail": (
            f"条纹数落在 {DEFAULTS['min_cycles']:g} 到 {PLATFORM_ACCURATE_CYCLES:g} 之间。"
            "峰还在，级次也没锁错，但峰形变宽，定峰的相对误差会到几个百分点 ——"
            "规范 §4.1 给的实测对照是 cycles 2.37 时偏差 -4.12%。\n\n"
            "**这一档的数值可以看趋势，不要拿去做定量比较。**"
            "想让它变准，唯一的办法是把分析窗口拓宽（窗口越宽，bin 越小，"
            "同样的膜里数得出的条纹越多）。"),
    },
    "LOW_CYCLES": {
        "label": "不可信",
        "short": f"条纹数不足 {DEFAULTS['min_cycles']:g}，FFT 会锁到噪声峰上",
        "detail": (
            f"窗口内连 {DEFAULTS['min_cycles']:g} 个完整条纹都数不出来。"
            "这时候功率谱上没有真正的主峰，argmax 找到的往往是噪声的起伏，"
            "**算出来的数字和膜厚没有关系**。\n\n"
            "常见原因：膜已经干到很薄（低于这个窗口的可测下限），"
            "或者窗口选得太窄。平台照样把数值给出来、不置空 ——"
            "判据只打标志、绝不修改数值 —— 但你不该用它。"),
    },
    "LOW_SNR": {
        "label": "不可信",
        "short": "主峰不够突出，可能是噪声",
        "detail": (
            f"峰值信噪比低于 {DEFAULTS['min_snr_db']:g} dB，"
            "主峰没有明显高过背景。常见于信号太弱、或者这一帧本身就没有条纹。"),
    },
    "UNDERSAMPLED": {
        "label": "不可信",
        "short": "每个条纹上的采样点太少",
        "detail": (
            f"平均每个条纹周期上不足 {DEFAULTS['min_pts_per_fringe']:g} 个采样点，"
            "条纹本身没有被采样清楚，谈不上定峰。"),
    },
}


def explain_status(status: str) -> dict:
    """给界面用的判级说明。找不到时如实说不认识，不编。"""
    return STATUS_TEXT.get(status, {
        "label": status, "short": "未知的判级", "detail": f"没有登记「{status}」这一档的说明。"})
